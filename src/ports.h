 ...
 * @file ports.h
 *
 * Macros for easy access to pins and their appropriate ports
 *
 ...
 * just as shown in the example above. The first argument is always expected
 * to describe the name of the port, whereas the second one describes the pin
 * number itself.
 *
 * Internally these macros exploit the fact that addresses of the involved
 * registers can be calculated. Normally the `PINx` register comes first and
 * is followed by the `DDRx` register, which in return is followed by the
 * `PORTx` register itself. For instance in case of the port C of an ATmega168
 * it looks like this (see [1]):
 *
 * \code
 *   [...]
 *  0x08 (0x28) PORTC
 ...
 * `0x00` instead of `0x60`, which would be expected. For details refer to [2].
 * However, this is also covered by the macros provided by this file.
 *
 * This implementation is a combination of ideas taken from [4] and [5].
 *
 * @note These macros are based on a concept known as [variadic macros][3].
 *
...
#ifndef _LTT_PORTS_H_
#define _LTT_PORTS_H_

/**
 * Returns the PORT register of an appropriate definition
 *
...
/**
 * Helper macro needed to implement PORT()
 *
...
/**
 * Returns the DDR register of an appropriate definition
 *
...
/**
 * Helper macro needed to implement DDR()
 *
 * @param a Name of the port
 * @param b Number of the pin
 *
...
/**
 * Returns the PIN register of an appropriate definition
 *
...
#define PIN(...) PIN_(__VA_ARGS__)
    ...
    /**
     * Helper macro needed to implement PIN()
     *
...
/**
 * Returns the pin number of an appropriate definition
 *
...
/**
 * Helper macro needed to implement BIT()
 *
 * @param a Name of the port
 * @param b Number of the pin
 *
